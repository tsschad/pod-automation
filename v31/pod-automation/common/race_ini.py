...
AI opponents, multiplayer REMOTE inactive):
  ...
  REMOTE section corrected (2026-08-09) against the actual code that
  writes it in the real world: AcTools (the library Content Manager
  itself is built on, same author) has a `Game.OnlineProperties.Set()`
  method in `AcTools/Processes/Game.Properties.cs` that is the ground
  truth for what a real multiplayer join writes into race.ini's
  [REMOTE] section before launching acs.exe:
  https://raw.githubusercontent.com/gro-ove/actools/master/AcTools/Processes/Game.Properties.cs

  That source showed the first draft here had two real mistakes,
  found only after a real hardware test produced "Failed to connect:
  handshake failed" when actually trying to join a server this way:
    - NAME and TEAM are not real keys in this section at all — driver
      name comes from [CAR_0].DRIVER_NAME (already set separately,
      unaffected by this fix), not from anything in [REMOTE]. The
      "NAME sets multiplayer display name" theory from the first draft
      was wrong; removed.
    - SERVER_NAME (a separate, genuinely real key — cosmetic, only
      written when a name is actually supplied) was missing entirely.
  GUID, PASSWORD, REQUESTED_CAR, SERVER_IP, SERVER_PORT,
  SERVER_HTTP_PORT, and ACTIVE were all already correct.

  Still an open question, not resolved by the above: whether the
  "handshake failed" error on the real hardware test was caused by
  these field-name mistakes, or by something else entirely (leading
  candidate: acs.exe launched directly, outside of Steam, may not have
  a valid Steamworks GUID available for the handshake unless Steam is
  actually running and logged in on that machine at launch time — see
  control-pc/README.md and phase1/README.md for what to check next).
  Both are worth ruling out; this fix alone may not be sufficient.
...
opponents) and this module can be tightened further — paste the file
back the same way you did this one.
"""
...
class RemoteConfig:
    ...
    via the REMOTE section. Field names confirmed against AcTools'
    actual Game.OnlineProperties.Set() (see module docstring) — NOT the
    same as a first-draft guess this project shipped earlier, which
    had two wrong keys (NAME, TEAM) and was missing SERVER_NAME.
    Driver display name is set separately via RaceConfig.driver_name
    (-> [CAR_0].DRIVER_NAME), not anything here."""
    server_ip: str
    server_http_port: int
    server_port: int
    server_name: str = ""         # cosmetic only; omitted entirely if blank, matching AcTools
    password: str = ""
    ...
    guid: str = ""                # Steam64 id; usually best left blank and let AC fill it in --
                                   # but see module docstring: this may require Steam to actually
                                   # be running/logged in on this machine to populate correctly
...
def build_race_ini(cfg: RaceConfig) -> str:
    ...
    if cfg.remote:
        ...
        if cfg.remote.server_name:
            _kv(lines, "SERVER_NAME", cfg.remote.server_name)
    else:
        _kv(lines, "ACTIVE", 0)
    lines.append("")
