...
from cm_launcher import join_online_server, join_online_server_direct  # noqa: E402
...
def api_mp_join():
    ...
    assigned acServer instance. Three join mechanisms, chosen via
    mp_config.json's "join_method":

    "direct" (the original approach): builds a race.ini with REMOTE
    populated and direct-launches acs.exe -- same write-then-launch
    mechanism as single-player. STATUS: tried on real hardware,
    failed with AC's "Failed to connect: handshake failed". A real
    working reference file (captured from Chad's existing venue
    management system, which also writes a config directly) showed
    REMOTE.ACTIVE=0 even for a live, working connection -- strong
    evidence this ACTIVE flag isn't actually what triggers a join at
    all. Kept available, not removed, since single-player's local
    (non-REMOTE) use of direct_launch is unaffected and proven
    working -- just not recommended for multiplayer anymore.

    "content_manager" (fallback/manual-test option): fires Content
    Manager's `acmanager://race/online/join` URI (see cm_launcher.py's
    join_online_server). CONFIRMED WORKING on real hardware -- the
    first successful multiplayer connection in this project. Only
    drawback: always opens Content Manager's server-info dialog and
    needs a manual "Join" click -- confirmed (via AcTools' actual
    source) there's no parameter to skip it. Doesn't carry the
    customer's car/track/name through either -- only ip/httpPort/
    password.

    "content_manager_direct" (recommended default): fires CM's
    `acmanager://race/online` URI instead -- a different action that
    connects immediately with no dialog, and does carry the
    customer's car/track/driver name through (see cm_launcher.py's
    join_online_server_direct). Not yet independently confirmed on
    hardware, but built from the same source research that confirmed
    "content_manager" works, using the identical underlying Steam/CM
    connection path -- just a different, non-interactive entry point.
    """
    body = request.get_json(force=True, silent=True) or {}
    ...
    cfg = get_config()
    mp_cfg = get_mp_config() or {}
    join_method = mp_cfg.get("join_method", "direct")

    if join_method == "content_manager":
        uri = join_online_server(
            ip=body["server_ip"],
            http_port=int(body["server_http_port"]),
            password=body.get("password", ""),
        )
        print(f"[mp/join] Opened Content Manager URI (dialog, manual Join click needed): {uri}", flush=True)
        _run_drive_click(cfg)
        return jsonify({"status": "launched", "method": "content_manager", "uri": uri})

    if join_method == "content_manager_direct":
        uri = join_online_server_direct(
            ip=body["server_ip"],
            port=int(body["server_port"]),
            http_port=int(body["server_http_port"]),
            car=body["car_id"],
            track=body.get("track_id", ""),
            name=body["customer_name"] or "Guest Driver",
            password=body.get("password", ""),
        )
        print(f"[mp/join] Opened Content Manager URI (direct, no dialog): {uri}", flush=True)
        _run_drive_click(cfg)
        return jsonify({"status": "launched", "method": "content_manager_direct", "uri": uri})
    ...
    return jsonify({"status": "launched", "method": "direct", "race_ini_path": written_path})
