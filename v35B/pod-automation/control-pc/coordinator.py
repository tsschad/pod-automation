...
alone. Confirmed working against a real instance (see
acserver_status.py's module docstring) -- no longer a guess.
...
Two tiers of acServer, not one -- because vanilla acServer.exe can
only run a single server per OS process (that part isn't a guess, it's
just how the software works), "one acServer instance running several
independent servers on different ports" has to mean something is
running multiple acServer.exe processes behind a single-looking
interface. For the 4 pooled `instances` in acserver_pool.json, that
something is this coordinator itself (see acserver_manager.py) --
matching what Chad's own "Assetto Corsa Server Manager" GUI tool
almost certainly does under the hood for its saved presets, just
without a GUI in front of it. But Chad also runs some servers of his
own through that GUI tool independent of any booking (a "Test Hotlap"
server, "Server 8", etc.) that this coordinator has no business
starting or stopping. Those go in acserver_pool.json's optional
`watched_servers` list instead: label + server_ip + http_port only,
queried the same way as the pooled instances (acserver_status.py) but
purely for display in the admin GUI -- no start/stop/config control,
since this coordinator doesn't own their lifecycle.

Run with:
    pip install -r requirements.txt
    python coordinator.py
Listens on 0.0.0.0:8090 by default. Admin GUI: http://<this-machine>:8090/admin

Port choice, not arbitrary: this used to default to 6000, which Chrome
and Firefox both refuse to connect to -- it's on their built-in list of
"unsafe" ports (a leftover association with X11), and fails with
ERR_UNSAFE_PORT with no indication the server itself is fine. Found
this the hard way against a real control PC: `curl.exe` worked, every
browser didn't, and `netstat`/the server logs showed nothing wrong.
8090 isn't on that blocked list. If you ever need to change the port
again, avoid: 1, 7, 9, 11, 13, 15, 17, 19, 20-23, 25, 37, 42, 43, 53,
69, 77, 79, 87, 95, 101-104, 109-111, 113, 115, 117, 119, 123, 135,
137, 139, 143, 161, 179, 389, 427, 465, 512-515, 526, 530-532, 540,
548, 554, 556, 563, 587, 601, 636, 989, 990, 993, 995, 1719, 1720,
1723, 2049, 3659, 4045, 5060, 5061, 6000, 6566, 6665-6669, 6697, 10080.
"""
...
INSTANCE_READY_TIMEOUT_SECONDS = 10
INSTANCE_READY_POLL_INTERVAL_SECONDS = 0.5
...
def _wait_for_instance_ready(server_ip: str, http_port: int) -> bool:
    """Poll an instance's own HTTP status endpoint (acserver_status.py)
    until it responds or INSTANCE_READY_TIMEOUT_SECONDS elapses. Called
    without holding _lock -- this can take several real seconds and
    shouldn't block other requests. Returns whether it became reachable
    in time; the caller currently proceeds either way (this is a
    best-effort delay to avoid telling pods to join too early, not a
    hard gate that blocks a session from starting)."""
    deadline = time.time() + INSTANCE_READY_TIMEOUT_SECONDS
    while time.time() < deadline:
        if acserver_status.query_instance_status(server_ip, http_port) is not None:
            return True
...
def api_start_session(group_id):
    ...
    with _lock:
        ...
        # Claim the instance right away, before waiting below, so a
        # concurrent start can't grab the same "free" instance in the
        # meantime -- _free_instance_id already excludes anything with
        # state == "active".
    ...
    # acServer.exe having just launched (restart_instance returning)
    # only means the OS process exists -- not that it's finished
    # initializing and is actually listening for game connections yet.
    # Telling pods to join immediately produced "Failed to connect:
    # handshake failed" on real hardware: the client's UDP handshake
    # reaching an instance that wasn't ready to accept it yet. Poll the
    # instance's own HTTP status endpoint (acserver_status.py, the same
    # thing the admin GUI uses) as a reasonably reliable proxy for "the
    # process has finished starting up" before telling anyone to
    # connect. Capped so a genuinely broken instance can't hang this
    # forever -- pods still get told to join once the cap is hit either
    # way, same as before this fix, just after less of a blind guess.
    # Done outside _lock so this multi-second wait doesn't block every
    # other request (other pods' status polling, admin GUI, etc.).
    _wait_for_instance_ready(server_ip, server_http_port)
...
def api_list_instances():
    """Live view of everything acServer-related, in two tiers (see
    module docstring):

    - `instances`: the 4 pooled instances this coordinator owns the
      lifecycle of, for customer bookings. Reports what's assigned to
      each (a customer group, an admin-started session, or nothing)
      plus a best-effort live status snapshot from that instance's own
      HTTP endpoint (acserver_status.py) -- car/track/session/
      connected drivers, when it's running and reachable.
    - `watched_servers`: anything else Chad runs himself (through his
      own Server Manager tool or otherwise) that he's listed in
      acserver_pool.json's `watched_servers` for visibility only --
      same live-status query, but this coordinator never starts, stops,
      or reconfigures these.
    """
    try:
        pool_cfg = _load_pool_config()
    except FileNotFoundError as exc:
    ...
    with _lock:
        ...
        watched_snapshot = list(pool_cfg.get("watched_servers", []))
    ...
    watched_result = []
    for i, w in enumerate(watched_snapshot):
        live_status = acserver_status.query_instance_status(
            w.get("server_ip", "127.0.0.1"), w.get("http_port")
        )
        watched_result.append({
            "label": w.get("label") or f"Server {i + 1}",
            "server_ip": w.get("server_ip", ""),
            "http_port": w.get("http_port"),
            "reachable": live_status is not None,
            "live_status": live_status,
        })

    return jsonify({"instances": result, "watched_servers": watched_result})
