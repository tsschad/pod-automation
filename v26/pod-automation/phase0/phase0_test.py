...
def cmd_mp_ini(args: argparse.Namespace) -> None:
    """Test the theory from race_ini.py: that a race.ini with REMOTE
    populated can join a server AND set the player's display name in
    one shot, via race/config instead of race/online/join."""
    cfg = RaceConfig(
