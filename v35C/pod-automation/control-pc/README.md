...
5. **Investigated, inconclusive on its own:** Chad opened AC's own LAN
   server browser directly on a pod and refreshed -- it found zero
   servers, despite a real acServer instance confirmed running.
   `../common/udp_probe.py` (a minimal UDP echo test, nothing
   AC-related) confirmed raw UDP genuinely isn't getting through
   between pod1 and the control PC, even with both machines' Windows
   Firewalls off. McAfee/third-party AV was ruled out (not installed).
   Chad's network is a Google Wifi mesh -- known to have real,
   undocumented quirks in how its stateful firewall handles UDP
   specifically (see the research notes from this session), though no
   single fixable "setting" was found. **This finding is still real
   and worth fixing for reliability**, but turned out not to be the
   reason multiplayer wasn't connecting -- see #6.

6. **The actual likely answer, found by comparing against a proven
   working config:** Chad already runs a separate venue management
   system (VMS) that successfully starts multiplayer AC sessions on
   this same network -- and it also writes a config directly (not
   through Content Manager's UI) rather than using Steam's normal
   launch flow. Chad captured its real `race.ini` right after a
   successful connection. Critically: **`[REMOTE] ACTIVE=0`**, even
   though every other REMOTE field (`SERVER_IP`, `SERVER_PORT`, GUID,
   etc.) was fully populated. That's strong evidence the working
   system does *not* rely on `REMOTE.ACTIVE=1` to trigger a join at
   all -- this project's whole "direct" approach may have been aimed
   at the wrong mechanism from the start, not broken by a field-name
   or timing bug. `phase1/app.py`'s `/api/mp/join` now supports a
   second mechanism, `"content_manager"` (via `mp_config.json`'s
   `join_method`), that fires Content Manager's
   `acmanager://race/online/join` URI instead -- a completely
   different, much simpler command than the one ruled out early in
   this project (`race/config`, which failed for an unrelated reason:
   encoding a whole race payload, not a bare connection). This was
   built in `common/cm_launcher.py` early on but never wired into the
   pod flow or actually tested. See `phase1/README.md`'s Multiplayer
   section for setup. **This is the next thing to test.**
...
1. Copy `acserver_pool.example.json` to `acserver_pool.json` and fill
   ...
   **Port collision fix (2026-08-09):** the example file's 4 pooled
   instances now default to ports 9641-9644 (UDP) / 8641-8644 (HTTP),
   not 9601-9604 / 8081-8084. The old numbers overlapped with Chad's
   own manually-run presets (e.g. "S1 - LMP1/GT3" on 9601/8081) — a
   "successful" test earlier actually connected to that pre-existing
   server instead of one this coordinator freshly configured. If your
   real `acserver_pool.json` still has the old 960x/808x ports, change
   its 4 `instances` entries to 9641-9644/8641-8644 (or any range
   above 9640 that doesn't overlap your own presets) and restart
   `coordinator.py`.
