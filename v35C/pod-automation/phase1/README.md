...
**Status: multiplayer join confirmed working on real hardware
(2026-08-09)**, via Content Manager -- not the original `race.ini`
`REMOTE.ACTIVE=1` direct-launch approach, which never worked (see
`../control-pc/README.md` for that troubleshooting history). This is
just the pod-side setup.

**Three join mechanisms, switchable via `mp_config.json`'s
`"join_method"`:**

- `"direct"` (the original approach): write a `race.ini` with its
  `REMOTE` section populated and launch `acs.exe` directly. **Failed**
  on real hardware every time ("Failed to connect: handshake failed")
  -- kept only because single-player's local (non-multiplayer) direct
  launch still uses the same underlying code path and that part works
  fine. Don't use this for multiplayer.
- `"content_manager"`: fires Content Manager's
  `acmanager://race/online/join` URI. **Confirmed working** on real
  hardware -- the first successful multiplayer connection in this
  project. Only drawback: it always opens Content Manager's
  server-info dialog and needs a manual "Join" click (confirmed, via
  Content Manager's own source, that there's no parameter to skip
  it) -- doesn't carry the customer's car/track/name through either,
  only IP/HTTP-port/password.
- `"content_manager_direct"` (current default): fires a different
  Content Manager URI, `acmanager://race/online`, which connects
  immediately with **no dialog and no manual click**, and does carry
  the customer's chosen car/track/driver name through. Built from the
  same source research that confirmed `"content_manager"` works, using
  the identical underlying Steam/CM connection path -- just a
  different, non-interactive entry point. Not yet independently
  confirmed on real hardware (next thing to test).

1. Set up and run `../control-pc/coordinator.py` first (see its
   README) -- and if your `acserver_pool.json` still has the old
   9601-9604/8081-8084 ports, move it to 9641-9644/8641-8644 first
   (see `../control-pc/README.md`'s port-collision note).
2. Copy `mp_config.example.json` to `mp_config.json` on this pod and
   fill in:
   ```json
   {
     "pod_id": "pod1",
     "control_pc_url": "http://192.168.1.50:8090",
     "join_method": "content_manager_direct"
   }
   ```
   `pod_id` must be unique per pod (`pod1`, `pod2`, etc.). Do this on
   every pod, each with its own `pod_id`, all pointing at the same
   control PC. If `content_manager_direct` doesn't connect cleanly on
   real hardware, fall back to `"content_manager"` (dialog + manual
   Join click, but proven) while that gets debugged.
...
**Testing with only one pod online:** the invite screen doesn't
require picking anyone — leaving the selection empty and confirming
starts a **solo hosted session** (a "group" of just that one pod,
auto-accepted, immediately ready to start). It still goes through the
exact same path as a real multi-pod session: the coordinator assigns a
pooled acServer instance, writes real config, restarts it, and tells
the pod to join via `POST /api/mp/join` — the same `race.ini` `REMOTE`
section + direct launch a real multiplayer session uses. This is the
easiest way to verify the actual join mechanism works on real hardware
before a second pod is even online, and doubles as a normal way to
run a solo session against a dedicated server (e.g. for testing or a
hotlap) rather than single-player's local session.
