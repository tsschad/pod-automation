...
function screenName() {
  const screen = renderScreen(`
    ...
    <div class="hint" id="name-hint">Press Enter to continue, or leave blank for "Guest"</div>
  `);
  const input = screen.querySelector("input");
  const hint = screen.querySelector("#name-hint");
  ...
  const submit = async () => {
    ...
    // mpReadyPromise resolves once /api/mp/config + registering with the
    // control PC both finish (or fail) -- waiting on it here avoids a
    // race where a fast tapper (or a cold first load right after app.py
    // starts -- config fetch + register + a CORS preflight round-trip)
    // reaches this screen before those network calls resolve, leaving
    // S.mpAvailable at its startup default of false and silently
    // skipping Multiplayer for no real reason. 8s cap so a genuinely
    // unreachable control PC can't hang this screen indefinitely --
    // falls back to single-player either way once it elapses. Once this
    // resolves once per page load, every later name entry in the same
    // browser session hits the `if` below as a no-op (S.mpAvailable is
    // already known), so this delay is only ever felt once.
    if (!S.mpAvailable) {
      hint.textContent = "Checking for multiplayer…";
      await Promise.race([mpReadyPromise, new Promise((resolve) => setTimeout(resolve, 8000))]);
    }

    // Pods without mp_config.json (or an unreachable control PC) never
    // offer Multiplayer -- straight to the same single-player flow.
    goto(S.mpAvailable ? screenMode : screenPack);
  };

  input.addEventListener("keydown", (e) => {
    if (e.key !== "Enter") return;
    // Stop this Enter from also reaching the document-level key handler
    // (see its own comment near the top of the file). Without this,
    // when submit() resolves synchronously (mpAvailable already known,
    // no await needed) and calls goto(screenMode) within this same
    // keydown dispatch, the still-bubbling event immediately re-fires
    // against screenMode's freshly-wired Enter handler too -- instantly
    // "pressing Enter" on its default-highlighted first option (Single
    // Player) before it's ever visibly rendered. Same event, two
    // handlers, one real keypress -- stopPropagation makes it one.
    e.stopPropagation();
    submit();
  });
...
async function screenMpInvitePods() {
  const screen = renderScreen(`
    ...
    <div class="hint" id="send-hint">Click pods to select them, then Send Invites. Only idle pods can be invited.</div>
  `);
  ...
  const hintEl = screen.querySelector("#send-hint");
  const selected = new Set();

  // Selecting nobody and continuing anyway still works -- it starts a
  // "solo" hosted session (a group of just this pod, auto-accepted,
  // immediately ready to start) that still goes through the real
  // acServer join path. Useful on its own for testing/hotlap, and
  // necessary right now while only one pod is online to verify the
  // multiplayer join mechanism without needing a second physical pod.
  function updateSendButton() {
    if (selected.size === 0) {
      sendBtn.textContent = "Start Solo Session";
      hintEl.textContent = pods.length === 0
        ? "No other pods online -- this will start a solo hosted session on just this pod."
        : "Click pods to select them, then Send Invites. Or leave none selected to start a solo hosted session on just this pod.";
    } else {
      sendBtn.textContent = "Send Invites";
      hintEl.textContent = "Click pods to select them, then Send Invites. Only idle pods can be invited.";
    }
  }
...
goto(screenWelcome);
// Doesn't block the welcome screen from showing -- but a fast tapper
// (or someone testing repeatedly) can genuinely reach name entry
// before these two sequential network round-trips (fetch /api/mp/config,
// then register with the control PC) finish resolving. screenName's
// submit handler awaits this promise (with a timeout) before deciding
// whether to show Multiplayer, rather than trusting a bare S.mpAvailable
// read that could still be false purely from a timing race.
const mpReadyPromise = initMultiplayer();
