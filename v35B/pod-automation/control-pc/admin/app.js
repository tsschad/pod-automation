...
const watchedSectionEl = document.getElementById("watched-section");
const watchedTbodyEl = document.getElementById("watched-tbody");
...
// Confirmed 2026-08-09 against a real running instance -- acServer's
// /INFO returns {ip, port, cport, name, clients, maxclients, track,
// cars, ...}. Used for a friendly one-line summary; the raw JSON is
// still shown underneath for anything this doesn't cover.
function summarizeLiveStatus(liveStatus) {
  if (!liveStatus || typeof liveStatus !== "object") return null;
  const parts = [];
  if (liveStatus.track) parts.push(liveStatus.track);
  if (liveStatus.clients !== undefined && liveStatus.maxclients !== undefined) {
    parts.push(`${liveStatus.clients}/${liveStatus.maxclients} clients`);
  }
...
function renderInstanceCard(instance) {
  ...
  const summary = summarizeLiveStatus(instance.live_status);
  if (summary) {
    const summaryEl = document.createElement("div");
    summaryEl.className = "live-summary";
...
function renderWatchedRow(server) {
  const tr = document.createElement("tr");
  const statusClass = server.reachable ? "pod-status-in_group" : "pod-status-idle";
  const statusLabel = server.reachable ? "online" : "not reachable";
  const summary = summarizeLiveStatus(server.live_status);
  tr.innerHTML = `
    <td>${server.label}</td>
    <td>${server.server_ip || "?"}:${server.http_port || "?"}</td>
...
async function refreshInstances() {
  try {
    ...
    const watched = data.watched_servers || [];
    watchedSectionEl.hidden = watched.length === 0;
    watchedTbodyEl.innerHTML = "";
    for (const server of watched) {
      watchedTbodyEl.appendChild(renderWatchedRow(server));
    }
  } catch (err) {
    setConnStatus(false);
  }
