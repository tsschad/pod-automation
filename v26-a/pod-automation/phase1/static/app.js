...
function screenName() {
  ...
  const submit = async () => {
    ...
    // control PC both finish (or fail) -- waiting on it here (briefly)
    // avoids a race where a fast tapper reaches this screen before those
    // two network round-trips resolve and S.mpAvailable is still its
    // startup default of false, silently skipping Multiplayer for no
    // real reason. Capped so an unreachable control PC can't hang this
    // screen indefinitely -- falls back to single-player either way.
    if (!S.mpAvailable) {
      hint.textContent = "Checking for multiplayer…";
      await Promise.race([mpReadyPromise, new Promise((resolve) => setTimeout(resolve, 3000))]);
    }
